...
export function ProjectsTab({ data, users, selectedUserId, months, onMonthsChange }: ProjectsTabProps) {
  ...
  const scoped = data.monthly.filter((item) => userIds.has(item.userId));
  const totalHours = scoped.reduce((sum, item) => sum + item.totalHours, 0);
  const bars = useMemo(() => data.projects.map((project) => {
  ...
  }).filter((project) => project.hours > 0).sort((a, b) => b.hours - a.hours), [data.projects, scoped, totalHours]);
...
function MonthlyBreakdown({ data, measurement, projectId, users, onClose }: { data: DashboardProjectsResponse; measurement: Measurement; projectId: string; users: User[]; onClose: () => void }) {
  ...
  const months = [...new Set(data.monthly.map((item) => item.month))].sort();
  const chartData = months.map((month) => {
    const point: Record<string, string | number> = { month: monthLabel(month) };
    const values = users.map((user) => data.monthly.find((item) => item.month === month && item.projectId === projectId && item.userId === user.id)?.[measurement === 'hours' ? 'totalHours' : 'totalPercent'] ?? 0);
    users.forEach((user, index) => { point[user.id] = Math.round(values[index] * 10) / 10; });
    point.average = values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : 0;
    return point;
  });
  return <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex justify-between"><h2 className="text-lg font-semibold">{project?.name ?? 'Prosjekt'} — månedlig fordeling</h2><button className="text-2xl text-slate-400 hover:text-slate-700" onClick={onClose} type="button" aria-label="Lukk">×</button></div><div className="mt-4 h-72"><ResponsiveContainer><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis unit={measurement === 'hours' ? 't' : '%'} /><Tooltip /><Legend />{users.map((user, index) => <Line dataKey={user.id} name={user.name} stroke={LINE_COLORS[index % LINE_COLORS.length]} strokeWidth={2} key={user.id} />)}<Line dataKey="average" name="Teamsnitt" stroke="#0f172a" strokeDasharray="5 4" strokeWidth={2} /></LineChart></ResponsiveContainer></div></section>;
}
...
function monthLabel(month: string) { return new Intl.DateTimeFormat('nb-NO', { month: 'short' }).format(new Date(`${month}-01T12:00:00`)); }
