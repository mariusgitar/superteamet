    ...
    const load = async () => {
      ...
      try {
        if (tab === 'week') {
          ...
          if (active) { setCurrentWeek(current); setPreviousWeek(previous); if (!users.length) setUsers(current.users); }
          ...
          if (active) { setPersonalEntries(entries.filter((entry) => entry.type === 'actual')); setTeamData(team); }
        }
      } catch (reason) {
